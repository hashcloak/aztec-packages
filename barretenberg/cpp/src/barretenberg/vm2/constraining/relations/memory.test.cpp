...
// lastAccess is derived from global_addr and global_addr'
TEST(MemoryConstrainingTest, LastAccess)
{
    TestTraceContainer trace({
        { { C::memory_sel_rng_chk, 1 },
          { C::memory_global_addr, 12345 },
          { C::memory_lastAccess, 1 },
          { C::memory_glob_addr_diff_inv, 1 } },
        { { C::memory_sel_rng_chk, 1 }, { C::memory_global_addr, 12346 }, { C::memory_lastAccess, 0 } },
        { { C::memory_sel_rng_chk, 1 }, { C::memory_global_addr, 12346 }, { C::memory_lastAccess, 0 } },
        { { C::memory_sel_rng_chk, 1 },
    ...
    // Mutate the trace to make the last access incorrect (lastAccess == 0 instead of 1).
    trace.set(C::memory_lastAccess, 0, 0);
    EXPECT_THROW_WITH_MESSAGE(check_relation<memory>(trace, memory::SR_LAST_ACCESS), "LAST_ACCESS");

    // Reset
    trace.set(C::memory_lastAccess, 0, 1);
    check_relation<memory>(trace, memory::SR_LAST_ACCESS);
    ...
    trace.set(C::memory_lastAccess, 2, 1);
    EXPECT_THROW_WITH_MESSAGE(check_relation<memory>(trace, memory::SR_LAST_ACCESS), "LAST_ACCESS");
}

// diff is derived as global_addr' - global_addr when lastAccess == 1.
TEST(MemoryConstrainingTest, DiffWithLastAccess)
{
    ...
    TestTraceContainer trace({
        ...
        { { C::memory_sel_rng_chk, 1 },
          ...
          { C::memory_rw, 1 } },
        { { C::memory_sel_rng_chk, 0 }, { C::memory_lastAccess, 0 } },
    });
...
// diff is derived as timestamp' - timestamp - rw' * rw when lastAccess == 0.
...
// Correct memory value (and tag) initialization after lastAccess == 1.
TEST(MemoryConstrainingTest, MemoryInitValueLastAccess)
{
    TestTraceContainer trace({
        { { C::memory_sel, 1 }, { C::memory_lastAccess, 1 } },
        { { C::memory_sel, 1 }, { C::memory_value, 0 }, { C::memory_tag, static_cast<uint8_t>(MemoryTag::FF) } },
    });
